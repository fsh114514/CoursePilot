...
1. 启动 CoursePilot 后，点击界面上的 **“请求屏幕录制权限”** 按钮，在弹出的系统授权框中允许 **CoursePilot**。
   - 如果没弹框，请手动到“系统设置 → 隐私与安全性 → 屏幕录制”中勾选 **CoursePilot**（注意：不是“Python”或“终端”，打包版是独立应用，需要单独授权）。
2. 授权后**重启 CoursePilot**（macOS 权限在启动时生效）。
3. 若“辅助功能”权限未开启，到“系统设置 → 隐私与安全性 → 辅助功能”中允许 CoursePilot 并重启。
4. 把课程窗口放到当前台前调度组，并点击“刷新窗口”。
5. 从列表选择课程窗口，确认预览正确。
6. 点击“开始监控”。
